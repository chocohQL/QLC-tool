...
public class CurveGroup<K, T, V> extends HashMap<K, ICurve<T, V>> implements ICurveGroup<K, T, V>{
    public static <K, T, V> CurveGroup<K, T, V> createGroup(List<T> data, Function<T, K> keyF) {
        return data == null ? null : createGroup(data.stream().filter(Objects::nonNull).collect(Collectors.groupingBy(keyF)));
    }
    ...
    public ICurveGroup<K, T, V> mergeGroup(ICurveGroup<K, T, V> group) {
        return keySetTraversal(group, (k, c1, c2) -> c1.mergeCurve(c2));
    }
    ...
    private static <K, T, V> CurveGroup<K, T, V> createGroup(Map<K, List<T>> group) {
        ...
        for (K k : group.keySet()) {
            curve.put(k, Curve.createCurve(group.get(k)));
        }
        return curve;
    }
}
